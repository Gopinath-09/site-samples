              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {sol.features.map((feat) => (
                  <div key={feat} className="rounded-xl border border-line/80 bg-sand/60 p-4 flex items-start gap-3">
                    <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-brand text-ink">
                      <Check width={14} height={14} />
                    </div>
                    <span className="text-xs font-semibold text-fg leading-tight">{feat}</span>
                  </div>
                ))}
              </div>
            </div>
