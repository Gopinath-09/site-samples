        ...
        {list.map((c) => (
          <RevealItem key={c.slug} className="h-full">
            <CardLink
              ...
              ariaLabel={c.title}
              className={cn(cardClasses({ interactive: true }), "flex h-full flex-col")}
            >
              ...
              <h3 className="heading-sm mt-5 text-fg">{c.title}</h3>
              <p className="body-sm mt-2 flex-1">{c.summary}</p>
              <div className="mt-6 grid grid-cols-3 gap-3 border-t border-line pt-5">
                {c.results.map((r) => (
                  <div key={r.label}>
                    <div className="text-base font-semibold text-fg">{r.value}</div>
